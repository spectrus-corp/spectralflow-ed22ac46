  ...
  const onSubmit = async (data: YTForm) => {
    ...
    try {
      await publishPost(user.id, {
        ...
        mediaType: "youtube",
        thumbnailUrl: ytId ? youTubeThumbnail(ytId) : null,
      });
      toast.success("Vidéo publiée !");
      onPosted();
    } catch (err) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      {ytId && (
        <div className="overflow-hidden rounded-2xl border border-border bg-black">
          <img src={youTubeThumbnail(ytId)} alt="Aperçu" className="aspect-video w-full object-cover" />
        </div>
      )}
      <Textarea
        rows={2}
        ...
        {...register("content")}
      />
      <Button type="submit" disabled={submitting || !ytId} className="w-full neon-glow">
        <Send className="mr-2 h-4 w-4" />
        {submitting ? "Publication…" : "Publier"}
      </Button>
    </form>
